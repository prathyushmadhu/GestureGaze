...
class FFFNerEncoder(tf.keras.layers.Layer):
  ...
  def __init__(
    ...
    self._pooler_layer_entity_type = tf.keras.layers.Dense(
        units=hidden_size,
        activation='tanh',
        kernel_initializer=tf_utils.clone_initializer(initializer),
    ...
    if with_dense_inputs:
      self.inputs = dict(
          ...
          is_entity_token_pos=tf.keras.Input(shape=(None,), dtype=tf.int32),
          entity_type_token_pos=tf.keras.Input(shape=(None,), dtype=tf.int32))
    else:
      self.inputs = dict(
          input_word_ids=tf.keras.Input(shape=(None,), dtype=tf.int32),
          input_mask=tf.keras.Input(shape=(None,), dtype=tf.int32),
